...
cfg = edict({
    'data_path': 'D:\\ai exp2\\flower_photos',
    'data_size':3670,
...
#测试集引入
testImagePath = 'D:\\ai exp2\\TestImages'
my_test = ds.ImageFolderDataset(testImagePath)
...
my_test = my_test.map(input_columns="image", num_parallel_workers=8, operations=transform_img)
my_test = my_test.map(input_columns="image", operations=hwc2chw_op, num_parallel_workers=8)
my_test = my_test.map(input_columns="image", operations=type_cast_op, num_parallel_workers=8)
my_test = my_test.shuffle(buffer_size=cfg.data_size)
#划分训练集测试集
de_train = []
de_test =[]
...
test_ = my_test.create_dict_iterator().__next__()
...
#使用测试集评估模型，打印总体准确率
    ...
    CKPT = os.path.join(cfg.output_directory,'checkpoint_classification_%d'%(i)+'-'+str(cfg.epoch_size)+'_'+str(de_train[i].get_dataset_size())+'.ckpt')
    net = Identification_Net(num_class=cfg.num_class, channel=cfg.channel, dropout_ratio=cfg.dropout_ratio)
    ...
    # test_ = de_test[i].create_dict_iterator().__next__()
    # test_ = my_test.create_dict_iterator().__next__()
    # test = Tensor(test_['image'], mindspore.float32)
    predictions = model[i].predict(test)
    ...
    for j in range(6):
        ...
        # print('第' + str(j) + '个sample预测结果：', class_names[pre_label], '   真实结果：', class_names[true_label[j]])
        record[j][pre_label] = record[j][pre_label] + 1
# print(record)
final_result = []
for i in range(6):
    max = 0
    ...
    for j in range(6):
        if record[i][j] > max:
            max = record[i][j]
            max_record = j
    final_result.append(max_record)

for i in range(6):
    print('第' + str(i) + '个sample预测结果：', class_names[final_result[i]], '   真实结果：', class_names[true_label[i]])